...
export default function JobHeatmap({ jobs = [] }) {
  ...
  useEffect(() => {
    ...
    jobs.forEach(job => {
      // 1. Database eken ena location eka lowercase karanawa
      const locationStr = job?.workLocation?.toLowerCase() || "";
      
      // 2. UPDATED LOGIC: 
      // Dan "exact match" hoyanne na. "Colombo 03" wage thibboth "colombo" kiyana eka hoyagannawa includes() eken.
      const matchedCityKey = Object.keys(CITY_COORDS).find(city => locationStr.includes(city));
      
      if (matchedCityKey) {
        if (!groups[matchedCityKey]) groups[matchedCityKey] = [];
        groups[matchedCityKey].push(job);
      }
    });

    console.log("Marked Cities on Map:", groups); // Debugging sadaha console ekata damma
            ...
            <Popup className="custom-popup">
              <div className="p-1 min-w-[200px]">
                ...
                <ul className="max-h-40 overflow-y-auto space-y-2">
                  {cityJobs.map(job => (
                    <li key={job._id || Math.random()} className="text-sm bg-gray-50 p-2 rounded hover:bg-purple-50 transition">
